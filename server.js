...
app.post('/api/chat', async (req, res) => {
  if (!anthropic) {
    return res.json({
      content: "AI chat isn't configured yet — add your ANTHROPIC_API_KEY to .env to enable it. Train tracking and live status work fine!"
    });
  }
  try {
...
const STATION_NAMES = {
  ...
  JHS:'Jhansi', DLI:'Old Delhi', RTM:'Ratlam', KOTA:'Kota',
  BRC:'Vadodara', ST:'Surat', UDZ:'Udaipur', NGP:'Nagpur',
  BZA:'Vijayawada', VSKP:'Visakhapatnam', RNC:'Ranchi',
  ERS:'Ernakulam', CBE:'Coimbatore', MDU:'Madurai',
  ASN:'Asansol', DHN:'Dhanbad', KGP:'Kharagpur',
  MGS:'Mughal Sarai', DDU:'Pt DD Upadhyaya Jn', MTJ:'Mathura',
  PRYJ:'Prayagraj', GWL:'Gwalior', ET:'Itarsi',
};

const STATION_NAME_TO_CODE = {
  'new delhi':'NDLS', 'ndls':'NDLS', 'h nizamuddin':'NZM',
  'hazrat nizamuddin':'NZM', 'old delhi':'DLI',
  'mumbai central':'BCT', 'bandra terminus':'BDTS',
  'chhatrapati shivaji terminus':'CSMT', 'csmt':'CSMT',
  'lokmanya tilak terminus':'LTT', 'dadar':'DR',
  'howrah jn':'HWH', 'howrah':'HWH', 'sealdah':'SDAH',
  'chennai central':'MAS', 'chennai egmore':'MS',
  'ksr bengaluru':'SBC', 'bengaluru':'SBC', 'bangalore':'SBC',
  'secunderabad jn':'SC', 'secunderabad':'SC', 'hyderabad':'HYB',
  'ahmedabad jn':'ADI', 'ahmedabad':'ADI',
  'pune jn':'PUNE', 'pune':'PUNE',
  'jaipur jn':'JP', 'jaipur':'JP',
  'lucknow jn':'LKO', 'lucknow':'LKO', 'lucknow nr':'LKO',
  'kanpur central':'CNB', 'kanpur':'CNB',
  'bhopal jn':'BPL', 'bhopal':'BPL',
  'gwalior jn':'GWL', 'gwalior':'GWL',
  'agra cantt':'AGC', 'agra':'AGC',
  'patna jn':'PNBE', 'patna':'PNBE',
  'guwahati':'GHY',
  'trivandrum central':'TVC', 'thiruvananthapuram':'TVC',
  'ernakulam jn':'ERS', 'ernakulam':'ERS',
  'coimbatore jn':'CBE', 'coimbatore':'CBE',
  'bhubaneswar':'BBS',
  'visakhapatnam':'VSKP', 'vizag':'VSKP',
  'ranchi':'RNC',
  'chandigarh':'CDG',
  'dehradun':'DDN',
  'varanasi jn':'BSB', 'varanasi':'BSB',
  'mughal sarai jn':'MGS', 'mughal sarai':'MGS',
  'pt dd upadhyaya jn':'DDU', 'dd upadhyaya jn':'DDU',
  'mathura jn':'MTJ', 'mathura':'MTJ',
  'prayagraj jn':'PRYJ', 'prayagraj':'PRYJ', 'allahabad':'ALD',
  'jhansi jn':'JHS', 'jhansi':'JHS',
  'nagpur jn':'NGP', 'nagpur':'NGP',
  'vadodara jn':'BRC', 'vadodara':'BRC',
  'surat':'ST',
  'udaipur':'UDZ',
  'vijayawada jn':'BZA', 'vijayawada':'BZA',
  'asansol jn':'ASN', 'asansol':'ASN',
  'dhanbad jn':'DHN', 'dhanbad':'DHN',
  'kharagpur jn':'KGP', 'kharagpur':'KGP',
  'ratlam jn':'RTM', 'ratlam':'RTM',
  'kota jn':'KOTA', 'kota':'KOTA',
  'itarsi jn':'ET', 'itarsi':'ET',
  'amritsar jn':'ASR', 'amritsar':'ASR',
  'gaya jn':'GAYA', 'gaya':'GAYA',
  'tatanagar jn':'TATA', 'tatanagar':'TATA',
  'madurai jn':'MDU', 'madurai':'MDU',
  'tirupati':'TPTY',
  'warangal':'WL',
  'dibrugarh':'DBRG',
  'puri':'PURI',
...
function parseDelay(delayStr) {
  if (!delayStr || delayStr === 'On Time' || delayStr === 'Source'
      || delayStr === 'Destination' || delayStr === '-') return 0;
...
function parseTiming(timing) {
  ...
  if (timing === 'Destination') return { arrival: timing, departure: 'Destination' };
  if (timing.length >= 10) {
    return {
  ...
  if (timing.length >= 5) {
    return { arrival: timing.substring(0, 5), departure: timing.substring(0, 5) };
  }
...
const TRAIN_DATABASE = [
  ...
  { train_number:'12049', train_name:'Gatimaan Express',              source:'NDLS', destination:'JHS'  },
  { train_number:'12050', train_name:'Gatimaan Express',              source:'JHS',  destination:'NDLS' },
...
app.get('/api/train/search', async (req, res) => {
  try {
    ...
    if (/^\d+$/.test(q)) {
      try {
        ...
        if (json && json.success && json.data?.length) {
          const rawName = (json.train_name || '').replace(' Running Status', '').replace(/^\d+\s*/, '');
          ...
          return res.json({ data: [{ train_number: q, train_name: rawName, source: src, destination: dest }] });
        }
      } catch (e) {
      ...
      const local = TRAIN_DATABASE.filter(t => t.train_number.startsWith(q));
    ...
    const matches = TRAIN_DATABASE.filter(t =>
      ...
      (STATION_NAMES[t.source] || '').toLowerCase().includes(ql) ||
      (STATION_NAMES[t.destination] || '').toLowerCase().includes(ql)
    );
...
app.get('/api/train/status/:trainNo', async (req, res) => {
  try {
    ...
    const normalizedStations = stations.map((stn, idx) => {
      ...
      return {
        station_code:        resolveCode(stn.station_name),
    ...
    log('STATUS', `${trainNo} — delay: ${delayMin}m, current: ${currentStn?.station_name}`);
    ...
    res.json({
      data: {
        ...
        current_station_info: {
          ...
          station_code: resolveCode(currentStn?.station_name || ''),
        },
...
app.get('/api/train/schedule/:trainNo', async (req, res) => {
  try {
    ...
    const currentIdx = json.data.findIndex(s => s.is_current_station === true);
...
app.get('/api/train/between', async (req, res) => {
  ...
  if (!from || !to) return res.json({ data: [], error: 'from and to required' });
  ...
  log('BETWEEN', `${from}→${to}: ${matches.length} trains`);
  res.json({ data: matches });
});
...
app.get('/api/weather', async (req, res) => {
  try {
    ...
    if (!lat || !lon) return res.json({ data: null, error: 'lat and lon required' });
    log('WEATHER', `${lat},${lon}`);
...
app.listen(PORT, () => {
  ...
  console.log(`   Claude AI : ${isRealKey ? '✅ Ready' : '⏳ Add ANTHROPIC_API_KEY later'}`);
