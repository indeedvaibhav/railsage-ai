...
export default function TrackerPage() {
  ...
  // Derive next station from schedule
  const currentStationIndex =
    trainSchedule?.stations?.findIndex((s) => s.status === "current") ?? -1;

  const nextStation =
    currentStationIndex >= 0 && trainSchedule?.stations
      ? trainSchedule.stations[currentStationIndex + 1]?.name || "—"
  ...
  const upcomingStations =
    trainSchedule?.stations
      ?.filter((s) => s.status === "upcoming" || s.status === "current")
      ?.slice(0, 5) || [];

  // Passed stations count
  const passedCount =
    trainSchedule?.stations?.filter((s) => s.status === "passed" || s.hasPassed)
      ?.length || 0;
  ...
  const delayColor =
    delayMinutes > 30 ? "#ef4444" : delayMinutes > 0 ? "#f59e0b" : "#22c55e";
      ...
      <div className="tracker-detail-pane">
        ...
        <div className="tracker-search-wrap">
          <div className="tracker-search-box">
            ...
            />
            {query && (
              <button className="tracker-search-clear" onClick={handleClear}>
                ✕
              </button>
            )}
          </div>
              ...
              {results.map((train) => (
                ...
                >
                  <div className="tracker-result-left">
                    <span className="tracker-result-number">
                      {train.trainNumber}
                    </span>
                    <span className="tracker-result-name">
                      {train.trainName}
                    </span>
                    <span className="tracker-result-route">
                      {train.from} → {train.to}
                    </span>
                  </div>
                  <span
                    className={`tracker-result-badge tracker-badge--${
          ...
          {showResults &&
            ...
            !loading && (
              <div className="tracker-search-dropdown">
                <p className="tracker-no-results">
                  No trains found for "{query}"
                </p>
              </div>
            )}
        </div>
        {/* Back button — only show when a train is selected */}
        {selectedTrain && (
            ...
            {allTrains.map((train) => (
              ...
              >
                <div className="tracker-train-row-left">
                  <span className="tracker-train-row-num">
                    {train.trainNumber}
                  </span>
                  <span className="tracker-train-row-name">
                    {train.trainName}
                  </span>
                  <span className="tracker-train-row-route">
                    {train.from} → {train.to}
                  </span>
                </div>
                <span
          ...
          <div className="tracker-detail">
            ...
            <div className="tracker-train-header">
              <div className="tracker-train-header-left">
                <span className="tracker-train-num">
                  {selectedTrain.trainNumber}
                </span>
                <span className="tracker-train-name">
            ...
            <div className="tracker-info-grid">
              ...
              <div className="tracker-info-box">
                ...
                <span className="tracker-info-value">{nextStation}</span>
              </div>
              <div className="tracker-info-box">
              ...
              <div className="tracker-info-box">
                ...
                <span className="tracker-info-value">
                  {(() => {
                    if (!selectedTrain.lastUpdated) return "Live";
                    const d = new Date(selectedTrain.lastUpdated);
                    return isNaN(d.getTime())
                      ? selectedTrain.lastUpdated // show raw string if not parseable
            ...
            {delayMinutes > 0 && (
              <div className="tracker-ai-suggestion">
                <div className="tracker-ai-suggestion-header">
                  <span className="tracker-ai-badge">
                    ⚡ RailSage Suggestion
                  </span>
                </div>
                      ...
                      <div key={station.code} className="tracker-station-row">
                        ...
                        <div className="tracker-station-info">
                          <div className="tracker-station-top">
                            <span
                              className={`tracker-station-name ${
